...
一个用于批量重命名压缩包文件的本地工具，提供可视化界面和丰富的重命名规则。
...
| 特性 | 说明 |
|------|------|
| 📁 目录扫描 | 支持递归扫描，自动识别压缩包文件 |
| 🧭 增强路径输入 | 支持常用目录、历史路径、本机目录补全和路径有效性校验 |
| 🖥️ 本地目录选择 | 通过本地后端打开系统文件夹选择器，返回完整目录路径 |
...
- 点击「浏览」会调用本地后端打开系统文件夹选择器，选择后自动填入完整路径并扫描
- 顶部路径输入框会根据窗口宽度自动伸缩，窄窗口下会换行显示

> 目录选择能力依赖本机运行的 FastAPI 后端。如果后端运行在远程服务器，文件夹选择窗口会出现在服务器所在机器上。
...
### 1. start.bat 窗口一闪而过 / 无响应

**原因**：Codex 桌面环境下 start.bat 内部的 start "..." cmd /k 弹出的控制台窗口可能被隐藏。

**解决**：切换到「方式二：手动启动」，分别在两个终端里执行后端和前端命令。

### 2. 前端启动报错 Failed to parse source for import analysis
